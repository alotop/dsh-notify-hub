...
/**
 * Which provider security features each webhook channel supports.
 *
 * A custom bot can be locked down three ways (飞书: 自定义关键词 / IP 白名单 /
 * 签名校验). Two of them are the caller's responsibility:
 *   * `keyword` — the message body must contain one of the configured keywords,
 *     or the provider answers `19024 Key Words Not Found`;
 *   * `signature` — the request must carry a timestamp and an HMAC signature, or
 *     the provider answers `19021` (飞书). DingTalk offers the same pair but puts
 *     the signature in the query string, so only 飞书 is listed for now;
 *     IP allowlisting needs nothing from us (the provider sees our source IP).
 *
 * Channels absent from this table get neither settings field, so the section
 * never offers a control the provider would ignore.
 */
export const WEBHOOK_SECURITY = Object.freeze({
  feishu: Object.freeze({ keyword: true, signature: true }),
})
