 ...
 *
 * On Windows the returned `script` is the PowerShell program to run: the caller
 * writes it to a temp file and passes `-File`, so nothing is ever piped into the
 * child. A piped stdin (the `-Command -` form) fails wherever a process may not
 * open a pipe, which is exactly what a sandboxed launcher enforces — and the
 * script carries the user's content as base64, so a plain ASCII `.ps1` file is
 * byte-exact without any encoding negotiation.
 *
 ...
 * @returns {{ command: string, args: string[], timeoutMs: number, script?: string } | null}
...
export function commandForPlatform(platform, envelope, settings) {
  ...
  if (platform === 'win32') {
    return {
      ...
      timeoutMs: 30_000,
      script: buildWindowsScript(rendered.title, rendered.body, envelope.id, settings.local?.sound !== false),
    }
  }
  if (platform === 'darwin') {
...
function runCommand(command, signal) {
  return new Promise((resolve, reject) => {
    ...
    let settled = false
    // No pipe on any of the three streams: this process only ever needs to
    // launch a notifier, and an unused pipe is a needless permission surface.
...
export async function sendLocalNotification(envelope, settings, options = {}) {
  ...
  // A Windows toast needs a program file; macOS/Linux carry theirs in argv.
  let scriptDir
  try {
    if (command.script !== undefined) {
      scriptDir = mkdtempSync(join(tmpdir(), 'dsh-notify-hub-'))
      const scriptPath = join(scriptDir, 'notify.ps1')
      // A BOM makes Windows PowerShell read the file as UTF-8 rather than as the
      // system code page, so future non-ASCII script text survives too.
      writeFileSync(scriptPath, `\uFEFF${command.script}`, 'utf8')
      command.args = [...command.args, '-File', scriptPath]
    }
    ...
    if (scriptDir !== undefined) {
      try {
        ...
        // A leftover temp file is not worth failing a delivered notification over.
      }
    }
  }
