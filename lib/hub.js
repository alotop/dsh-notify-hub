    ...
    switch (channel) {
      ...
      default: {
        ...
        const result = await deliverWebhook(channel, channelSettings.url, envelope, {
          ...
          // 飞书 bot security: 自定义关键词 must appear in the body, and 签名校验
          // needs the per-request sign fields. Both are no-ops when unset.
          keyword: channelSettings.keyword,
          secret: channelSettings.secret,
          secrets: [channelSettings.url, channelSettings.secret],
        })
        return `attempt(s)=${result.attempts}`
      }
    }
    ...
    const task = deliver(channel, envelope, settings)
      ...
      .catch((error) => {
        const secrets = [
          ...
          ...WEBHOOK_CHANNELS.flatMap((name) => [
            settings.webhooks[name]?.url,
            settings.webhooks[name]?.secret,
          ]),
        ]
